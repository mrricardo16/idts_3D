...
| 缺失 | Scene、Movable Part、Motion Target API；正式 API Client；前后端联调；转换流水线；PostgreSQL/API/前端集成测试与 CI。 |
