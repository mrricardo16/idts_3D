...
推荐路线为先完成模型资产和 GLB 设备闭环，再完成可动部件配置和场景设备绑定；随后接入单设备 WCS 状态，验证状态映射、坐标映射和告警闭环；再扩展 AGV、堆垛机、提升机、输送线和货位；并行推进 3D Tiles 厂区底座的坐标和性能验证；最后完成生产化、权限、审计、部署、运维和远程控制专项评审。
