...
## DOC-PLAN-07：MVP-10A 完整任务书门禁

MVP-10A 总卡与 01～05 是正式混合接入的任务书，不是实施结果：10A-01 冻结并审核 `baseLayers + devices`、数据来源、版本/兼容/回滚；10A-02 建图层骨架；10A-03 用获批 POC 结论接入 Tiles/坐标；10A-04 同步正式数据与跨端契约；10A-05 以真实证据完成回退、性能、生命周期总验收。POC-3DT-01 仍仅为 Ready for user execution approval，MVP-10A 与子卡仍为 Blocked；字段、数据库、库版本、坐标和性能未获批准时必须记录 TBD，不得猜测实施。
