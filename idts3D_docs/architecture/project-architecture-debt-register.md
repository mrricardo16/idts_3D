...
| BE-DEBT-010 | Closed / P1；ARCH-03A 已建立 Architecture Tests，程序集引用和类型签名自动保护 Domain/Application/Contracts 依赖方向、Controller 与 Application Service 边界、Domain/Contracts 纯净性及测试项目引用方向；不覆盖方法体 IL 或完整源码分析；新增跨层依赖时触发复检；ARCH-03A；否/否/否/否；完整 Architecture Tests；删除测试基础设施可回退。 |
